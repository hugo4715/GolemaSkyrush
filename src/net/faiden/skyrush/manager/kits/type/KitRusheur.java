...
public class KitRusheur extends KitAbstract {
	...
	public void sendKits(Player player) {
		...
		player.getInventory().addItem(new ItemBuilder().type(Material.STAINED_CLAY)
				.data((byte) gamePlayer.getTeamInfo().getDataClay()).amount(158).build());
	...
	public ItemStack getItemIcons(Player player) {
		return new ItemBuilder()
				...
				.lore("", "§7Description :", " §7● §f1 Armure en cuir", " §7● §f1 Epée en fer", " §7● §f1 Pioche en pierre", " §7● §f158 Blocs",
						" §7● §f6 Steaks", "", "§6» §eCliquez pour utiliser.")
				.amount(GolemaPlayer.getGolemaPlayer(player).getSkyRushPlayer().getPlayerKits(SkyRushKitsMode.SKYRUSH_2TEAMS, SkyRushKitsType.RUSHEUR))
				.build();
	}
}